<?php

return [
	'title' => 'SMW Central - Votre resource primaire pour le hacking Super Mario World',
	'intro' => [
		'title' => 'Bienvenue sur SMW Central!',
		'beginners' => 'Besoin d\'aide pour jouer aux hacks que nous offrons? Vous n\'êtes pas sûr c\'est quoi le « ROM hacking »? Notre {link}Guide pour débutants{/link} vous montrera la route.', // U+00AB left-pointing double angle quotation mark     U+00BB right-pointing double angle quotation mark
	],
	'files' => [
		'title' => [
			'new' => 'Dernier contenu',
			'featured' => 'Contenu du jour',
		],
		'info' => '{Submission} | Soumis par {username} le {date}',
		'read_more' => '(lire plus…)', // U+2026 (HORIZONTAL ELLIPSIS)
	],
	'news' => [
		'title' => 'Dernières nouvelles',
		'sticky' => 'Épinglée:',
		'info' => 'Publiée par {username} le {date}',
		'all' => 'Toutes les nouvelles',
	],
	'calendar' => [
		'title' => 'Calendrier',
		'js' => 'Veuillez activer le JavaScript pour voir le calendrier.',
		'previous' => 'Précédent',
		'next' => 'Suivant',
		'events_today' => 'Événements aujourd\'hui:',
		'events_on' => 'Événements le {date}:',
		'all_events' => 'Tous les événements',
		'separator' => ' - ',
		'period' => '{from} – {to}', // U+2013 (EN DASH)
		'birthdays' => 'Liste des anniversaires',
		'empty_today' => 'Il n\'y a aucun événement aujourd\'hui.',
		'empty_on' => 'Il n\'y a aucun événement ce jour-ci',
	],
	'submission' => [
		'tools' => 'Outil',
		'documents' => 'Document',
		'brrsamples' => 'Échantillon BRR',
		'smwhacks' => 'SMW | Hack',
		'sramdatabase' => 'SMW | SRAM',
		'smwgraphics' => 'SMW | Graphismes',
		'smwmusic' => 'SMW | Musique',
		'smwblocks' => 'SMW | Bloc',
		'smwsprites' => 'SMW | Sprite',
		'smwpatches' => 'SMW | Patch',
		'uberasm' => 'SMW | UberASM',
		'yihacks' => 'YI | Hack',
		'yimusic' => 'YI | Musique',
		'yipatches' => 'YI | Patch',
		'spasm' => 'YI | SPASM',
		'sm64hacks' => 'SM64 | Hack',
		'sm64music' => 'SM64 | Musique',
		'sm64textures' => 'SM64 | Texture',
	],
];
